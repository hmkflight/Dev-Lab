...
export interface RealPlanInput {type:string;projectSlug:string;clientName?:string;expectedGate?:string;note?:string;runId?:string;approvedBy?:string;selectedTrack?:string;gateUpdatedAt?:string;definition?:{intake:Record<string,unknown>;implementation:unknown};}
export function realCommandPlan(input:RealPlanInput,root:string) {
 ...
 if(input.type==='APPROVE_GATE'&&(input.expectedGate==='AWAITING_DIRECTION_APPROVAL'?!['editorial','experiential','experimental'].includes(input.selectedTrack||''):input.selectedTrack!==undefined))throw Error('Direction selection missing or not applicable.');
 if(input.type==='APPROVE_GATE'&&['AWAITING_DIRECTION_APPROVAL','AWAITING_BUILD_APPROVAL','AWAITING_FINAL_APPROVAL'].includes(input.expectedGate||'')&&/^[a-f0-9-]{36}$/.test(input.runId||'')&&input.approvedBy)return {cwd:root,file:resolve(root,'command.sh'),args:['creative-studio','approve','--project',input.projectSlug,'--run-id',input.runId!,'--expected-stage',input.expectedGate!,'--approval-type',input.expectedGate!,'--approved-by',input.approvedBy,...(input.selectedTrack?['--selected-track',input.selectedTrack]:[]),...(input.gateUpdatedAt?['--gate-updated-at',input.gateUpdatedAt]:[])]};
