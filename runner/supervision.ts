...
export function launchTask(job:BridgeCommand,executionId:string,base:string){
 ...
 const child=spawn(process.execPath,['--conditions=react-server','--import','tsx',resolve('runner/real-task.ts'),file],{cwd:process.cwd(),shell:false,detached:true,stdio:'ignore',env:{BRIDGE_PROJECT_SLUG:process.env.BRIDGE_PROJECT_SLUG,TSX_TSCONFIG_PATH:resolve(FACTORY_ROOT,'dashboard/tsconfig.json'),PATH:process.env.PATH,HOME:process.env.HOME,USER:process.env.USER,LANG:process.env.LANG,TMPDIR:process.env.TMPDIR}});
