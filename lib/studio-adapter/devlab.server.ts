...
export class DevLabStudioAdapter implements StudioAdapter {
  ...
  private async detail(id:string):Promise<ProjectDetail> {
    ...
    if(run?.human_gate&&p.current_stage===run.human_gate) approvals.unshift({id:`${run.id}:${run.human_gate}`,projectId:p.id,title:text(run.human_gate),description:'CPE is waiting for human approval. Studio controls are disabled.',kind:text(run.human_gate),status:'pending',createdAt:run.updated_at});
    ...
    return {project,stages,agents,approvals,productionRun:run?{id:run.id,projectId:p.id,status:text(run.status),stageId:text(run.current_stage),startedAt:run.started_at,updatedAt:run.updated_at,mode:this.mode,iterationNumber:run.current_production_iteration,responsibleAgent:text(run.responsible_agent),lastAgent:text(events.find(e=>e.event_type==='AGENT_INVOKED'&&typeof e.detail?.agent==='string')?.detail.agent)}:null,
