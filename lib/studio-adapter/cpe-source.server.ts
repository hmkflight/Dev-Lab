...
const tables = new Set(['studio_project_authorizations','creative_studio_projects','creative_studio_production_runs','creative_studio_production_run_events','creative_studio_artifacts','creative_studio_production_iterations','creative_studio_reviews','approvals','agents','labs']);
...
export class SupabaseReadSource implements ReadSource {
  constructor(private url?: string, private key?: string, private transport: typeof fetch = (input, init) => fetch(input, init), private readToken?: string) {}
  async rows(table: string, query: Record<string,string>): Promise<Row[]> {
    if (!this.url || (!this.key && !this.readToken)) throw new DevLabAdapterNotConfiguredError();
    ...
    const response = this.readToken ? await this.transport(new URL('/functions/v1/studio-read',base),{method:'POST',headers:{Authorization:`Bearer ${this.readToken}`,'Content-Type':'application/json'},body:JSON.stringify({table,query}),redirect:'manual',signal:AbortSignal.timeout(15000)}) : await this.transport(new URL('/rest/v1/'+table+'?'+new URLSearchParams({...query,limit:query.limit || '200'}),base),{
