...
export const commandInput=z.union([
 ...
 z.object({...base,projectId:projectSlug,mode:z.literal('REAL'),type:z.literal('RESUME_RUN'),payload:z.object({...fence,expectedProjectUpdatedAt:z.string().datetime({offset:true}).optional()}).strict()}).strict(),
 z.object({...base,projectId:projectSlug,mode:z.literal('REAL'),type:z.literal('APPROVE_GATE'),payload:z.object({...fence,expectedProjectUpdatedAt:z.string().datetime({offset:true}).optional(),selectedTrack:z.enum(['editorial','experiential','experimental']).optional(),approvalType:z.enum(['AWAITING_DIRECTION_APPROVAL','AWAITING_BUILD_APPROVAL','AWAITING_FINAL_APPROVAL']),gate:z.enum(['AWAITING_DIRECTION_APPROVAL','AWAITING_BUILD_APPROVAL','AWAITING_FINAL_APPROVAL'])}).strict()}).strict(),
]).refine(d=>d.mode!=='REAL'||d.type!=='APPROVE_GATE'||(d.payload.gate==='AWAITING_DIRECTION_APPROVAL'?!!d.payload.selectedTrack:d.payload.selectedTrack===undefined),'Direction approvals require a selected track; other gates must not include one');
...
export function semanticKey(d:CommandInput){if(d.mode==='MOCK')return null;const p=d.payload as {runId?:string;expectedStage?:string;expectedProjectUpdatedAt?:string};return [d.projectId,d.type,p.runId||'',p.expectedStage||'',...(p.expectedProjectUpdatedAt?[p.expectedProjectUpdatedAt]:[])].join(':');}
