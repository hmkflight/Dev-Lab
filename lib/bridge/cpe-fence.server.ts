...
export class CpeFence {
 ...
 async validate(d:CommandInput,lockHolder?:string){
  ...
  const state=await this.state(),{project,run}=state;
  ...
  }else if(run.status!=='AWAITING_HUMAN_APPROVAL'||project.current_stage===run.current_stage||project.current_stage!=='EXPERIENCE_DESIGN')throw new StudioError('Resume requires the approved direction gate on this run.',409);
  return state;
 }
}
export const cpeFence=(env:{DEVLAB_SUPABASE_URL?:string;DEVLAB_SUPABASE_SERVICE_ROLE_KEY?:string})=>new CpeFence(new SupabaseReadSource(env.DEVLAB_SUPABASE_URL,env.DEVLAB_SUPABASE_SERVICE_ROLE_KEY));
