...
import {REAL_PROJECT,authorizedProject,type CommandInput} from './validation.server';
...
export class CpeFence {
 ...
 async validate(d:CommandInput,lockHolder?:string){
  ...
  if(d.type==='CREATE_PROJECT'){const state=await this.state();if(state.project)throw new StudioError('Project already exists.',409);return state;}
...
export const cpeFence=(env:{DEVLAB_SUPABASE_URL?:string;DEVLAB_SUPABASE_SERVICE_ROLE_KEY?:string;DEVLAB_READ_TOKEN?:string;BRIDGE_PROJECT_SLUG?:string})=>new CpeFence(new SupabaseReadSource(env.DEVLAB_SUPABASE_URL,env.DEVLAB_SUPABASE_SERVICE_ROLE_KEY,undefined,env.DEVLAB_READ_TOKEN),env.BRIDGE_PROJECT_SLUG||REAL_PROJECT);
