...
export interface BridgeCommand {
  ...
  history:string; execution_id?:string|null; process_id?:number|null; progress_at?:string|null;
}
export interface FactoryPresence {runnerId:string;mode:'MOCK'|'REAL';online:boolean;lastSeen:string;staleAfterSeconds:number;}
export interface BridgeSnapshot {runner:FactoryPresence|null;commands:BridgeCommand[];canSubmit:boolean;mode:'MOCK'|'REAL';real?:{projectSlug:string;allowed:string[];proven:string[];capabilities?:{canCreateProject:boolean;canStartRun:boolean;canResumeRun:boolean;canApprove:boolean;canPauseRun:false;canCancelRun:false};projectId?:string;runId?:string;stage?:string;runStatus?:string;gate?:string|null;};artifacts?:{id:string;sourceArtifactId:string;mimeType:string;bytes:number;sha256:string;previewUrl:string;downloadUrl:string}[];}
