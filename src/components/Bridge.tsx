...
export function Bridge(){
  ...
  return <>
    <div className="page-title"><div><span className="eyebrow">PASS 2 · DISPOSABLE CONTROL</span><h1>Factory bridge<span>.</span></h1><p>Real CPE observation and controls fenced to one disposable project. Other projects remain read-only.</p></div></div>
...
function RealControls({state,refresh}:{state:BridgeSnapshot;refresh:()=>Promise<unknown>}){
 ...
 const enabled=(type:string)=>state.canSubmit&&!busy&&real.allowed.includes(type);
 ...
 <button className="button primary" disabled={!enabled('APPROVE_GATE')||real.gate!=='AWAITING_DIRECTION_APPROVAL'||real.stage!==real.gate} onClick={()=>void send('APPROVE_GATE')}>Approve fenced direction</button>
 <button className="button primary" disabled={!enabled('RESUME_RUN')||real.stage!=='EXPERIENCE_DESIGN'||real.runStatus!=='AWAITING_HUMAN_APPROVAL'} onClick={()=>void send('RESUME_RUN')}>Resume disposable run</button>
 <button className="button secondary" disabled={busy||!last.current||!state.canSubmit} onClick={()=>void send()}>Repeat real request</button></div>
 <p>Unproven commands are available only in this explicit acceptance console. Pause and cancellation are disabled.</p>
