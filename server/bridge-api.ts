...
export async function bridgeAPI(request:Request,env:BridgeEnv&{DB:D1Database;BUCKET:R2Bucket},actor:{role:string;id:string},readBody:(r:Request)=>Promise<unknown>):Promise<Response>{
  ...
  if(request.method==='GET'&&path==='/status'){
    ...
    const state=allowed.length?await cpeFence(env).state():undefined;
    const objects=await env.DB.prepare('SELECT * FROM bridge_artifacts WHERE project_id=?').bind(REAL_PROJECT).all<any>();
    return json({mode:allowed.length?'REAL':'MOCK',real:{projectSlug:REAL_PROJECT,allowed,proven,capabilities:{canCreateProject:proven.includes('CREATE_PROJECT'),canStartRun:proven.includes('START_RUN'),canResumeRun:proven.includes('RESUME_RUN'),canApprove:proven.includes('APPROVE_GATE'),canPauseRun:false,canCancelRun:false},projectId:state?.project?.id,runId:state?.run?.id,stage:state?.project?.current_stage,runStatus:state?.run?.status,gate:state?.run?.human_gate},artifacts:objects.results.map(a=>({id:a.id,sourceArtifactId:a.source_artifact_id,mimeType:a.mime_type,bytes:a.bytes,sha256:a.sha256,previewUrl:`/api/bridge/artifacts/${a.id}`,downloadUrl:`/api/bridge/artifacts/${a.id}?download=1`})),runner:await q.presence(env.BRIDGE_RUNNER_ID||'hudson-mac-shadow'),commands:actor.role==='owner'?await q.list():[],canSubmit:actor.role==='owner'});
  }
  ...
  if(/^\/artifacts\/[a-f0-9-]{36}$/.test(path)&&request.method==='GET'){
    ...
    return new Response(object.body as unknown as ReadableStream,{headers:{'Content-Type':a.mime_type,'Content-Security-Policy':"default-src 'none'; sandbox",'X-Content-Type-Options':'nosniff','Cache-Control':'private, no-store',...(url.searchParams.has('download')?{'Content-Disposition':'attachment; filename="disposable-artifact.json"'}:{})}});
  }
