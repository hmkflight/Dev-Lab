...
import {commandInput,outcomeSchema,REAL_PROJECT,realTypes,authorizedProject} from '../lib/bridge/validation.server';
...
export async function bridgeAPI(request:Request,env:BridgeEnv&{DB:D1Database;BUCKET:R2Bucket},actor:{role:string;id:string},readBody:(r:Request)=>Promise<unknown>):Promise<Response>{
  const url=new URL(request.url),path=url.pathname.replace('/api/bridge','');const projectSlug=env.BRIDGE_PROJECT_SLUG||REAL_PROJECT;authorizedProject(projectSlug,'REAL',projectSlug);const q=new BridgeQueue(env.DB,undefined,projectSlug);const json=(v:unknown,status=200)=>Response.json(v,{status,headers:{'Cache-Control':'no-store'}});
  ...
  if(path==='/commands'&&request.method==='POST'){
    ...
    if(data.mode==='REAL'){authorizedProject(data.projectId,'REAL',projectSlug);if(data.type==='CREATE_PROJECT')await cpeFence(env).state();else await cpeFence(env).authorize();}
  ...
  if(/^\/artifacts\/[a-f0-9-]{36}$/.test(path)&&request.method==='GET'){
    const a=await env.DB.prepare('SELECT * FROM bridge_artifacts WHERE id=? AND project_id=?').bind(path.split('/')[2],projectSlug).first<any>();
    if(a)await cpeFence(env).authorize();
    const object=a&&await env.BUCKET.get('bridge-real/'+a.id);if(!object)throw new StudioError('Artifact not published.',404);
    return new Response(object.body as unknown as ReadableStream,{headers:{'Content-Type':a.mime_type,'Content-Security-Policy':"default-src 'none'; style-src 'unsafe-inline'; img-src data:; sandbox",'X-Content-Type-Options':'nosniff','Cache-Control':'private, no-store, no-transform',...(url.searchParams.has('download')?{'Content-Disposition':`attachment; filename="disposable-artifact.${({'application/json':'json','image/png':'png','image/jpeg':'jpg','text/html':'html'} as Record<string,string>)[a.mime_type]||'bin'}"`}:{})}});
  }
