...
export async function publishDisposableArtifact(input:unknown,env:BridgeEnv&{DB:D1Database;BUCKET:R2Bucket}){
 ...
 const fence=cpeFence(env),{project}=await fence.authorize();if(!project)throw new StudioError('Disposable project not found.',404);
 ...
 const content=new TextDecoder('latin1').decode(bytes);if(Object.entries(env).some(([name,value])=>/TOKEN|SECRET|KEY/.test(name)&&typeof value==='string'&&value.length>15&&content.includes(value)))throw new StudioError('Artifact contains private transport information.',400);
