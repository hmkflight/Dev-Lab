...
export async function publishDisposableArtifact(input:unknown,env:BridgeEnv&{DB:D1Database;BUCKET:R2Bucket}){
 const b=z.object({sourceArtifactId:z.uuid(),mimeType:z.literal('application/json'),base64:z.string().max(700000),sha256:z.string().regex(/^[a-f0-9]{64}$/)}).strict().parse(input);
 ...
 const content=new TextDecoder('utf-8',{fatal:true}).decode(bytes);if(/\/Users\/|\/home\/|file:\/\/|Bearer\s|(?:api[_-]?key|service_role|access_token|shellCommand)\s*["':=]/i.test(content))throw new StudioError('Artifact contains private transport information.',400);
 try{JSON.parse(content);}catch{throw new StudioError('Only valid non-sensitive JSON artifacts are supported.',400);}
 ...
 const prior=await env.DB.prepare('SELECT * FROM bridge_artifacts WHERE source_artifact_id=?').bind(b.sourceArtifactId).first<any>();if(prior){if(prior.sha256!==hash)throw new StudioError('Published source is immutable.',409);return prior;}
 const id=crypto.randomUUID();await env.BUCKET.put('bridge-real/'+id,bytes);
 await env.DB.prepare('INSERT INTO bridge_artifacts(id,source_artifact_id,project_id,mime_type,bytes,sha256,created_at) VALUES(?,?,?,?,?,?,?)').bind(id,b.sourceArtifactId,REAL_PROJECT,b.mimeType,bytes.length,hash,new Date().toISOString()).run();
 return {id,sourceArtifactId:b.sourceArtifactId,mimeType:b.mimeType,bytes:bytes.length,sha256:hash};
}
