...
export class BridgeQueue {
  constructor(private db:D1Database,private now=()=>new Date().toISOString(),private projectSlug=REAL_PROJECT){}
  async submit(input:unknown,actor:string){
    const data=commandInput.parse(input);authorizedProject(data.projectId,data.mode,this.projectSlug);
