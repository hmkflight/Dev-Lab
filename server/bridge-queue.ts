...
import { commandInput,authorizedProject,SYNTHETIC_PROJECT } from '../lib/bridge/validation.server';
...
export class BridgeQueue {
  ...
  async submit(input:unknown,actor:string){
    const data=commandInput.parse(input);authorizedProject(data.projectId);
    const fingerprint=JSON.stringify([data.projectId,data.type,data.mode,data.payload]);const now=this.now();
    await this.db.prepare(`INSERT OR IGNORE INTO bridge_commands (id,project_id,command_type,mode,payload,requested_by,idempotency_key,fingerprint,created_at,status,attempt_count,available_at,history) VALUES (?,?,?,?,?,?,?,?,?,'QUEUED',0,?,?)`).bind(crypto.randomUUID(),data.projectId,data.type,data.mode,JSON.stringify(data.payload),actor,data.idempotencyKey,fingerprint,now,now,JSON.stringify([{status:'QUEUED',at:now}])).run();
    const job=await this.db.prepare('SELECT * FROM bridge_commands WHERE requested_by=? AND idempotency_key=?').bind(actor,data.idempotencyKey).first<Job>();
  ...
  async heartbeat(runner:string,online=true){const now=this.now();await this.db.prepare(`INSERT INTO bridge_runners (id,last_seen,mode,online) VALUES (?,?,'MOCK',?) ON CONFLICT(id) DO UPDATE SET last_seen=excluded.last_seen,mode=excluded.mode,online=excluded.online`).bind(runner,now,online?1:0).run();return {lastSeen:now};}
  async presence(runner:string){const r=await this.db.prepare('SELECT * FROM bridge_runners WHERE id=?').bind(runner).first<{id:string;last_seen:string;mode:'MOCK';online:number}>();return r?{runnerId:r.id,mode:r.mode,lastSeen:r.last_seen,online:r.online===1&&Date.parse(this.now())-Date.parse(r.last_seen)<45000,staleAfterSeconds:45}:null;}
  async claim(runner:string){
    ...
    await this.db.prepare(`UPDATE bridge_commands SET status='FAILED',error='Lease expired after execution started; manual review required.',completed_at=?,history=json_insert(history,'$[#]',json_object('status','FAILED','at',?)) WHERE status='RUNNING' AND lease_until<?`).bind(now,now,now).run();
    ...
    return this.db.prepare(`UPDATE bridge_commands SET status='CLAIMED',claimed_by=?,claimed_at=?,claim_token=?,lease_until=?,attempt_count=attempt_count+1,history=json_insert(history,'$[#]',json_object('status','CLAIMED','at',?)) WHERE id=(SELECT id FROM bridge_commands WHERE status='QUEUED' AND mode='MOCK' AND project_id=? AND available_at<=? AND attempt_count<3 ORDER BY created_at LIMIT 1) AND status='QUEUED' RETURNING *`).bind(runner,now,crypto.randomUUID(),until,now,SYNTHETIC_PROJECT,now).first<Job>();
  }
  ...
  async finish(id:string,token:string,runner:string,outcome:{ok:boolean;retryable:boolean;code:'MOCK_COMPLETED'|'MOCK_TRANSIENT_FAILURE'|'MOCK_PERMANENT_FAILURE'|'EXECUTION_UNCERTAIN';executionCount:number}){
    ...
    if(job.status!=='RUNNING'||job.lease_until<=this.now())throw new StudioError('Command is not running under this lease.',409);
    const retry=!outcome.ok&&outcome.retryable&&outcome.code==='MOCK_TRANSIENT_FAILURE'&&outcome.executionCount===0&&job.attempt_count<3;
    const state=outcome.ok?'SUCCEEDED':retry?'QUEUED':'FAILED',now=this.now();
    const result=JSON.stringify({mode:'MOCK',code:outcome.code,executionCount:outcome.executionCount,cpeInvoked:false,stdout:outcome.ok?'Synthetic command completed; no CPE invocation.':'',stderr:outcome.ok?'':outcome.code});
