...
export const bridgeCommands = sqliteTable('bridge_commands', {
  ...
  semantic_key:text('semantic_key'), execution_id:text('execution_id'), process_id:integer('process_id'), progress_at:text('progress_at'),
...
export const bridgeArtifacts=sqliteTable("bridge_artifacts",{id:text("id").primaryKey(),source_artifact_id:text("source_artifact_id").notNull(),project_id:text("project_id").notNull(),mime_type:text("mime_type").notNull(),bytes:integer("bytes").notNull(),sha256:text("sha256").notNull(),created_at:text("created_at").notNull()},t=>[uniqueIndex("bridge_artifacts_source").on(t.source_artifact_id)]);
