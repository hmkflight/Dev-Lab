...
test('hosted state survives independent requests and rejects stale writes',async()=>{
 ...
 const a=await loadStudio(env.DB), b=await loadStudio(env.DB); await a.adapter.pauseRun(project.id); await a.persistence!.save(); await b.adapter.pauseRun(project.id); await assert.rejects(b.persistence!.save(),/another request/);
});
test('hosted uploads download original bytes and enforce same origin',async()=>{
 ...
 const result=await worker.fetch(new Request(upload.url,{method:'POST',headers:{Authorization:'Bearer test-owner','Content-Type':upload.headers.get('content-type')!,'Content-Length':String(bytes.byteLength),Origin:'https://studio.test'},body:bytes}),env);
