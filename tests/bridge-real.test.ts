...
function fixture(){const project={id:crypto.randomUUID(),slug:REAL_PROJECT,status:'active',current_stage:'AWAITING_DIRECTION_APPROVAL'};const run={id:runId,project_id:project.id,status:'AWAITING_HUMAN_APPROVAL',current_stage:'AWAITING_DIRECTION_APPROVAL',human_gate:'AWAITING_DIRECTION_APPROVAL',lock_holder:null,lock_expires_at:null};const approvals:any[]=[];return {project,run,approvals,fence:new CpeFence({rows:async table=>table==='creative_studio_projects'?[project]:table==='creative_studio_production_runs'?[run]:approvals})};}
