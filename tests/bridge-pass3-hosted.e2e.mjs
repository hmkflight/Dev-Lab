...
try{
 ...
 if(['build','resume'].includes(phase)){
  ...
  assert.equal(before.runner.online,false);await expect(page.getByRole('button',{name:'Approve Build',exact:true})).toBeDisabled();await expect(page.getByRole('button',{name:'Resume disposable run',exact:true})).toBeDisabled();record.offlineControlsDisabled=true;
